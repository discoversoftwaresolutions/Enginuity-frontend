...
/** Lazy placeholders for routes (keeps bundle lean) */
...
const MODULES: ModuleDef[] = [
  ...
  },
  {
    ...
    path: "/circuitiq",
    description:
      "Professional electronic design automation platform with circuit simulation, PCB layout, and signal integrity analysis",
  ...
  },
  {
    ...
    path: "/codemotion",
    description:
      "Industrial robotics programming and motion control platform with advanced path planning and safety systems",
...
function Sidebar() {
  const { pathname } = useLocation();
  return (
    <aside className="w-80 min-h-screen bg-slate-900/80 backdrop-blur-sm border-r border-slate-700 p-6 sticky top-0">
      <div className="mb-8">
            ...
            <NavLink
              ...
              className={cls(
                ...
                active
                  ...
                  : "bg-slate-800/50 hover:bg-slate-800/70 border-slate-700 text-slate-200"
              )}
            >
              ...
              <p className="text-slate-300 text-xs mb-2">{m.description}</p>
              <div className="flex flex-wrap gap-1">
                ...
                {m.tags.length > 2 && (
                  <span className="px-2 py-1 text-xs text-slate-400">+{m.tags.length - 2}</span>
                )}
              </div>
            </NavLink>
          );
...
function DashboardMain() {
  ...
  return (
    <div className="flex-1 p-8 bg-white">
      <div className="mb-8">
        <h1 className="text-4xl font-bold text-gray-900 mb-4">Engineering Intelligence Suite</h1>
        <p className="text-xl text-gray-700 mb-2">
          Advanced computational tools for aerospace and engineering applications
        </p>
        <p className="text-gray-600">
          Professional-grade simulation, analysis, and design optimization platform
        </p>
      </div>
        ...
        <div className="lg:col-span-2">
          <div className="bg-white border border-gray-200 rounded-lg p-4 shadow-sm">
            ...
            <div className="space-y-3">
              {metrics.map((m) => (
                <div key={m.name} className="flex items-center justify-between py-2 border-b border-gray-100 last:border-b-0">
                  <div className="flex items-center space-x-3">
                    <span
                      className={cls(
                        "text-lg",
                        m.status === "operational" ? "text-green-500" : m.status === "maintenance" ? "text-yellow-500" : "text-red-500"
                      )}
                    >
                  ...
                  <div className="text-right">
                    <div className="text-sm text-gray-600">Uptime: {m.uptime}</div>
                    <div className="text-sm text-gray-500">Load: {m.load}</div>
                  </div>
                </div>
              ))}
            </div>
        ...
        <div className="space-y-6">
          <div className="bg-white border border-gray-200 rounded-lg p-4 shadow-sm">
            ...
            <div className="space-y-4">
              {[
                { label: "CPU Utilization", pct: 67, tint: "bg-blue-500" },
                { label: "Memory Usage", pct: 52, tint: "bg-green-500" },
                { label: "Network I/O", pct: 23, tint: "bg-yellow-500" }
              ].map((b) => (
                <div key={b.label}>
                  ...
                  <div className="w-full bg-gray-200 rounded-full h-2">
                    <div className={cls(b.tint, "h-2 rounded-full")} style={{ width: `${b.pct}%` }} />
                  </div>
                </div>
              ))}
            </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mt-12">
        {[
          ...
          <div key={s.label} className="bg-white border border-gray-200 rounded-lg p-6 text-center shadow-sm">
            <div className="text-2xl font-bold text-gray-900 mb-2">{s.value}</div>
                ...
                <div className="flex-1 p-8 bg-white">
                  <div className="max-w-2xl">
                    ...
                    <p className="text-gray-600 mb-4">
                      The page you’re looking for doesn’t exist. Return to the dashboard or choose a module from the left.
                    </p>
                    <Link
                      to="/"
